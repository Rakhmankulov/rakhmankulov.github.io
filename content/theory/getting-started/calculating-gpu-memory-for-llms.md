...
- **Битовая точность**. Формат точности (например, FP16, FP8, INT8) влияет на объём занимаемой памяти. Меньшая точность существенно снижает объём памяти, но может повлиять на точность вывода. Подробности — в [LLM quantization](./llm-quantization).
